...
void make_dir_and_file(directory_t* root_dir, char** token_list, int num_token) {
	...
	if (num_token) {
		...
		for (index = 0; index < num_token - 1; index++) {
			is_create = false;
			dir = find_create_dir(parent_dir, token_list[index], &is_create);
			if (is_create)
				parent_dir->dir_list[parent_dir->num_dir++] = dir;
			parent_dir = dir;
	     	}
		
		is_create = false;

		file = find_create_file(parent_dir, token_list[num_token - 1], &is_create);
		if (is_create)
			parent_dir->file_list[parent_dir->num_file++] = file;
      }
}
